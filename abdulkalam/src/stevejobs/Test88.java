...
public class Test88
{
	public static void main(String[] args) throws Exception
	{
		...
		driver.findElement(By.xpath("//*[text()='Next']"))
		                                             .click();
		Test88 obj=new Test88(); //object for current class
		if(uid.length()==0)
		{
			try
			{
			...
			catch(Exception ex)
			{
				...
				et.log(LogStatus.FAIL,"Blank UID test failed"+
				   ex.getMessage()+et.addScreenCapture(x));
			}
		}
		else if(uidc.equals("invalid"))
		{
			...
			{
				String x=obj.screenshot(driver);
			  et.log(LogStatus.FAIL,"Invalid UID test failed"+
				   ex.getMessage()+et.addScreenCapture(x));
			}
		}
		else
		{
			try
			{
				...
				if(pwd.length()==0)
				{
					...
					{
						String x=obj.screenshot(driver);
						et.log(LogStatus.FAIL,"Blank PWD test failed"+
						  ex.getMessage()+et.addScreenCapture(x));
					}
				}
				else if(pwdc.equals("invalid"))
				{
					...
					{
						String x=obj.screenshot(driver);
					  et.log(LogStatus.FAIL,"Invalid PWD test failed"+
						    ex.getMessage()+et.addScreenCapture(x));
					}
				}
				else
				{
					...
					{
						String x=obj.screenshot(driver);
				et.log(LogStatus.FAIL,"Valid PWD test failed"+
					   ex.getMessage()+et.addScreenCapture(x));
					}
				}
			}
	...
	public String screenshot(ChromeDriver driver)
	                           throws Exception
	{
